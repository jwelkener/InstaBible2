    ...
    return new Promise((resolve, reject) => {
        ...
        validateImage(imageUrl)
            .then(() => {
                ...
                // Resolve the promise to signal that the image is ready
                resolve();
            })
            .catch((error) => {
...
verseInputField.addEventListener('input', function () {
    const verseInput = verseInputField.value;
    const imageInput = imageInputField.value;
    if (isValidVerseFormat(verseInput) && imageInput) {
        // Fetch and display the verse only if the image is already present
        fetchVerseAndDisplay(verseInput, imageInput).catch((error) => {
...
imageInputField.addEventListener('input', function () {
    const imageInput = imageInputField.value;
    // Display the image
    displayImage(imageInput).catch((error) => {
        // Handle error as needed
